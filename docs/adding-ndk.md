...
which means full functional use goes through `xCon/sysbios/random.cfg` + regeneration.
...
1. In `tools/vendor-sdk.sh`, add an NDK section (mirror the existing `copy_ext`
   ...
   `xCon/third_party/sdk/ndk/packages/ti/ndk/inc/`.
2. Add `xCon/third_party/sdk/ndk/packages` to the `sdk_config` include dirs in
   `CMakeLists.txt`.
...
1. **Vendor headers + libs.** Extend `vendor-sdk.sh` to also copy the chosen
   `*.am4fg` libs under `xCon/third_party/sdk/ndk/.../lib/`. Pick the variant set:
...
3. **Enable NDK in `xCon/sysbios/random.cfg` and regenerate.** This is the key step — NDK is
   ...
   - **emit the correct NDK `*.am4fg` set into `xCon/generated/linker.cmd`'s
