...
# SYS/BIOS static configuration from xCon/sysbios/random.cfg for the GNU Arm target.
#
# After this runs and you commit xCon/generated/random_pm4fg.c + linker.cmd,
...
# xCon/sysbios/random.cfg (tick period, heap size, statically-defined Hwis, etc.).
#
...
OUT_DIR="${PROJ_DIR}/xCon/generated"
CFG="${PROJ_DIR}/xCon/sysbios/random.cfg"
# ---------------------------------------------------------------------------
...
# reent-model hooks newlib actually calls live in xCon/sysbios/newlib_locks.c. Idempotent.
...
if [ -f "${GEN_CFG}" ] && ! grep -q 'LOCAL PATCH: obsolete newlib __libc_lock' "${GEN_CFG}"; then
    ...
    perl -0777 -i -pe 's{(\nvoid __libc_lock_init\(_LOCK_T \*lock\)\n.*?\n\}\n)(\n/\*\n \* ======== ti\.sysbios\.rts\.gnu\.SemiHostSupport TEMPLATE)}{\n#if 0 /* LOCAL PATCH: obsolete newlib __libc_lock_* glue; see xCon/sysbios/newlib_locks.c */$1#endif /* LOCAL PATCH */$2}s' "${GEN_CFG}"
fi

# The generated linker.cmd is used only for its SECTIONS/symbols (layered on top
# of xCon/linker/tm4c129encpdt.lds). Its INPUT(...) block pulls the configuro-built
...
# libraries from xCon/third_party/, so strip the INPUT() block. Idempotent.
